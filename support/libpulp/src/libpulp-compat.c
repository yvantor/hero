...
int hero_dev_mbox_read(const HeroDev *dev, uint32_t *buffer, size_t n_words) {
  return pulp_mbox_read((const pulp_dev_t *)dev->dev, buffer, n_words);
}
int hero_dev_mbox_write(HeroDev *dev, uint32_t word) {
  return pulp_mbox_write((pulp_dev_t *)dev->dev, word);
}
...
int hero_dev_mmap(HeroDev *dev) {
  ...
  dev->dev = malloc(sizeof(pulp_dev_t));
  pulp_dev_t *pulp = (pulp_dev_t *)dev->dev;
  ret = pulp_mmap(pulp, "/dev/pulp0");
  if (ret)
    return ret;

  // Assign L3 memory to RTL struct
  dev->l3_mem.v_addr = pulp->l3.v_addr;
  dev->l3_mem.p_addr = (size_t)pulp->l3.p_addr;
  dev->l3_mem.size = pulp->l3.size;
...
int hero_dev_munmap(HeroDev *dev) {
  ...
  pulp_mbox_write(pulp, MBOX_DEVICE_STOP);
...
int hero_dev_init(HeroDev *dev) {
  ...
  fesrv_init(gd.fs, pulp, &a2h_rb_addr);
  ...
  memtest(pulp->l1.v_addr, pulp->l1.size, "TCDM", 'T');

  return 0;
}


static void set_direct_tlb_map(pulp_dev_t *pulp, uint32_t idx, uint32_t low, uint32_t high) {
...
static void reset_tlbs(pulp_dev_t *pulp) {
  struct axi_tlb_entry tlb_entry;
  tlb_entry.flags = 0;
  tlb_entry.first = 0;
  tlb_entry.last = 0;
...
void hero_dev_reset(HeroDev *dev, unsigned full) {
  ...
  reset_tlbs(pulp);
  set_direct_tlb_map(pulp, 0, 0x0000000, 0xffffffff); // Whole address space

  (void)pulp_reset(pulp);
}
...
void hero_dev_exe_start(HeroDev *dev) {
  ...
  bd = pulp_l3_malloc(pulp, sizeof(*bd), &bd_phys);

  // Populate bootdata
  bd->core_count = pulp->sci.compute_num;
  ...
  bd->global_mem_start = (uint32_t)(uintptr_t)(pulp->sci.l3_paddr + pulp->l3_data_offset);
  bd->global_mem_end = (uint32_t)(uintptr_t)(pulp->sci.l3_paddr + pulp->l3.size);
  // TODO: Handle multi-cluster
  bd->cluster_count = 1;
  // Let's pretend all clusters were in the same quadrant
  bd->s1_quadrant_count = 1;
  bd->clint_base = pulp->sci.clint_base;
  // unused
  bd->boot_addr = (uint32_t)(uintptr_t)pulp->sci.l3_paddr;

  // Set entry-point, bootrom pointer and l3 layout struct pointer
  pulp_exe_start(pulp,(uint32_t)(uint64_t)pulp->l3.p_addr);

  pr_trace("Data in L1:\n");
  hexdump(pulp->l1.v_addr, 0x30);

  // Set log level from env
  pulp_set_device_loglevel(pulp, -1);
...
uintptr_t hero_dev_l3_malloc(HeroDev *dev, unsigned size_b, uintptr_t *p_addr) {
  return (uintptr_t)pulp_l3_malloc((pulp_dev_t *)dev->dev, size_b, (void *)p_addr);
}
...
void hero_dev_l3_free(HeroDev *dev, uintptr_t v_addr, uintptr_t p_addr) {
  pulp_l3_free((pulp_dev_t *)dev->dev, (void *)v_addr, (void *)p_addr);
}
