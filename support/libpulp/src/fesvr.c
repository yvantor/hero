 ...
 * Intended to use for only a single pulp cluster
 *
 */
...
void fesrv_init(fesrv_t *fs, pulp_dev_t *dev, void **a2h_rb_p) {
  ...
  fs->a2h_rb = (struct ring_buf *)pulp_l3_malloc(fs->dev, sizeof(struct ring_buf), a2h_rb_p);
  assert(fs->a2h_rb);
  fs->a2h_rb->data_v =
      (uint64_t)pulp_l3_malloc(fs->dev, a2hrb_element_size * a2hrb_elements, &addr);
    ...
    if (ret == 0) {
      ...
      if (((prev_tail + 1) % fs->a2h_rb->size) != fs->a2h_rb->tail) {
        ...
        pr_error("This usually means the buffer area was overwritten. Perhaps you didn't allocate "
               "enough\n");
        pr_error("memory in the last call to pulp_l3_malloc before fesrv_init?\n");
      }
      prev_tail = fs->a2h_rb->tail;
 ...
 * @details
 *
 * @param magicMem magic memory passed from pulp to host
 */
static void handleSyscall(fesrv_t *fs, uint64_t magicMem[6]) {
  bool handled = false;
